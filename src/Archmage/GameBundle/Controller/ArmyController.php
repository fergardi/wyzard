...
class ArmyController extends Controller
{
    ...
    public function attackQuest($quest) {
        ...
        for ($i = 0; $i < $rounds; $i++) {
            ...
            $attackerSkill = ($attackerTroop->getUnit()->getSkill() ? " <span class='label label-skill' data-html='true' data-toggle='tooltip' data-placement='top' title='".$attackerTroop->getUnit()->getSkill()->getDescription()."'>".$attackerTroop->getUnit()->getSkill()->getName()."</span>" : "");
            $defenderSkill = ($defenderTroop->getUnit()->getSkill() ? " <span class='label label-skill' data-html='true' data-toggle='tooltip' data-placement='top' title='".$defenderTroop->getUnit()->getSkill()->getDescription()."'>".$defenderTroop->getUnit()->getSkill()->getName()."</span>" : "");
            ...
            elseif ($attackerSpeed > $defenderSpeed) {
        ...
        if (!$wipe) {
            ...
            $text[] = array($player->getFaction()->getClass(), 11, 0, 'center', 'Has ganado '.$this->get('service.controller')->nff($quest->getRunes()).' <span class="label label-artifact"><a href="'.$this->generateUrl('archmage_game_kingdom_market').'" class="link">Runa(s)</a></span>.');
